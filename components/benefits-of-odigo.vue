<template>
    <div class="benefits-of-odigo">
        <h3>
            Benefits of Odigo
        </h3>
        <div class="benefits-of-odigo__advantages-wrapper">
            <div class="benefits-of-odigo__advantages">
                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                        viewBox="0 0 511.998 511.998" style="enable-background:new 0 0 511.998 511.998;" xml:space="preserve">
                    <g>
                        <g>
                            <path d="M510.794,285.265c0.004,0-38.823-59.193-38.828-59.202c-0.675-1.028-1.649-1.914-2.729-2.498l-68.204-36.54v-28.138
                                c0,0,35.552,0,35.571,0c4.143,0,7.501-3.358,7.501-7.501c0-2.005-0.786-3.827-2.068-5.173l-50.181-59.948
                                c12.273-6.058,23.578-14.108,33.303-23.912c9.401-9.478,9.33-25.241-0.141-34.634c-9.478-9.4-25.246-9.325-34.635,0.141
                                c-14.141,14.256-33.766,22.431-53.843,22.431H175.46c-20.078,0-39.703-8.176-53.843-22.431
                                c-9.394-9.471-25.174-9.526-34.634-0.142c-9.471,9.395-9.524,25.175-0.142,34.634c9.724,9.804,21.029,17.854,33.303,23.912
                                l-50.482,60.305c-1.87,2.234-2.277,5.35-1.044,7.989c1.232,2.639,3.882,4.326,6.796,4.326h35.553v28.138
                                c0,0-68.181,36.527-68.213,36.545c-1.077,0.604-2.049,1.467-2.727,2.501c0,0-38.793,59.142-38.827,59.193
                                c-3.214,4.901,0.432,11.614,6.272,11.614h31.317v186.907c0,4.142,3.358,7.501,7.501,7.501h29.124c4.142,0,7.501-3.358,7.501-7.501
                                v-28.77h53.448v28.77c0,4.142,3.357,7.501,7.501,7.501h29.124c4.143,0,7.501-3.358,7.501-7.501v-28.77h53.447v28.77
                                c0,4.142,3.357,7.501,7.501,7.501h29.124c4.143,0,7.501-3.358,7.501-7.501v-28.77h53.447v28.77c0,4.142,3.357,7.501,7.501,7.501
                                h29.124c4.143,0,7.501-3.358,7.501-7.501v-28.77h53.448v28.77c0,4.142,3.357,7.501,7.501,7.501h29.124
                                c4.143,0,7.501-3.358,7.501-7.501V296.879c0,0,31.258,0,31.316,0C510.383,296.879,514.016,290.176,510.794,285.265z M97.492,51.79
                                c-1.781-1.797-2.679-4.258-2.67-6.721c0.028-8.256,10.308-12.525,16.144-6.643c16.937,17.075,40.443,26.869,64.494,26.869h161.078
                                c24.05,0,47.556-9.793,64.494-26.869c3.641-3.671,9.753-3.69,13.418-0.054c3.664,3.635,3.682,9.761,0.054,13.418
                                c-10.681,10.769-23.527,19.135-37.497,24.682c-0.016,0.006-0.034,0.009-0.05,0.015c-12.768,5.063-26.676,7.785-40.419,7.785
                                H175.46c-13.783,0-27.619-2.665-40.418-7.785c-0.019-0.008-0.039-0.01-0.058-0.018C121.016,70.923,108.172,62.558,97.492,51.79z
                                M91.476,143.885l43.152-51.549c0.125,0.044,0.253,0.08,0.378,0.123c12.953,4.483,26.745,6.814,40.454,6.814
                                c0,0,161.078,0,161.08,0c13.715,0,27.441-2.331,40.404-6.798c0.142-0.049,0.285-0.089,0.427-0.139l43.151,51.549H320.507
                                l-60.967-32.662c-2.213-1.186-4.872-1.186-7.085,0l-60.967,32.662H91.476z M386.029,158.887v20.101l-37.521-20.101H386.029z
                                M163.49,158.886l-37.52,20.101v-20.101H163.49z M53.791,476.285V296.879h14.123v179.406H53.791z M165.487,476.285h-14.122
                                v-21.269h14.122V476.285z M263.06,476.285h-14.122v-21.269h14.122V476.285z M360.633,476.285H346.51v-21.269h14.122V476.285z
                                M429.082,440.015H82.916v-14.123h346.166V440.015z M82.916,410.891V296.879h151.02v114.012H82.916z M248.938,410.891V296.879
                                h14.122v114.012H248.938z M318.401,339.884v28.003h-40.339v-28.003H318.401z M278.062,324.882v-28.003h40.339v28.003H278.062z
                                M318.401,382.888v28.003h-40.339v-28.003H318.401z M373.742,339.884v28.003h-40.339v-28.003H373.742z M333.403,324.882v-28.003
                                h40.339v28.003H333.403z M373.742,382.888v28.003h-40.339v-28.003H373.742z M429.083,410.891h-40.339v-28.003h40.339V410.891z
                                M429.083,367.887h-40.339v-28.003h40.339V367.887z M429.083,324.882h-40.339v-28.003h40.339V324.882z M458.207,476.285h-14.122
                                V296.879h14.122V476.285z M21.363,281.877l28.992-44.2h100.596c4.143,0,7.501-3.358,7.501-7.501s-3.357-7.501-7.501-7.501H76.189
                                l179.81-96.331l179.81,96.331H180.953c-4.143,0-7.501,3.358-7.501,7.501s3.357,7.501,7.501,7.501h280.689l28.992,44.2H21.363z"/>
                        </g>
                    </g>
                </svg>
                <h4>
                    Welcome to Odigo!
                </h4>
                <h5>
                    Jump off balcony, onto stranger's head. Chase ball of string hide when guests come over.
                </h5>
                <a href="#" class="benefits-of-odigo__advantages-link" @mouseover="learnMore1 = true" @mouseleave="learnMore1 = false" @click="learnMore1 = false">
                    <span>
                        learn more
                    <span class="benefits-of-odigo__advantages-link-border" :class="{'main__menu-links-border-animation' : learnMore1, 'main__menu-links-border-animation-leave' : learnMore1 === false}"></span>
                    </span>
                    <svg viewBox="0 0 33 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M24.7706 3H0V5H24.7706V8L33 4L24.7706 0V3Z" fill="#006DFE"/>
                    </svg>
                </a>

            </div>
            <div class="benefits-of-odigo__advantages">
                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                        viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
                    <g>
                        <g>
                            <path d="M421.054,414.843c-4.142,0-7.5,3.358-7.5,7.5v70.514c0,2.283-1.858,4.141-4.141,4.141h-40.317V349.301
                                c0-4.142-3.358-7.5-7.5-7.5c-4.142,0-7.5,3.358-7.5,7.5v147.698h-81.185l23.543-25.9c2.572-2.83,3.785-6.861,3.244-10.787
                                c-0.01-0.076-0.022-0.152-0.035-0.228L277.24,327.617l6.041-9.094c3.34,2.372,5.913,4.656,10.738,4.656
                                c4.908,0,9.497-2.747,11.755-7.269v-0.001l23.65-47.4l53.876,20.865c1.949,0.836,30.252,13.582,30.252,47.238v50.73
                                c-0.001,4.141,3.357,7.5,7.5,7.5c4.142,0,7.5-3.358,7.5-7.5v-50.73c0-44.344-37.969-60.463-39.585-61.128
                                c-0.047-0.02-0.095-0.039-0.143-0.057l-89.668-34.726v-21.03c14.242-11.076,24.117-27.495,26.596-46.227
                                c7.101-0.5,13.69-3.152,19.071-7.779c7.027-6.043,11.059-14.838,11.059-24.126c0-7.708-2.781-15.068-7.737-20.803V92.953
                                C348.144,41.699,306.446,0,255.192,0c-51.254,0-92.952,41.699-92.952,92.953v28.511c-5.009,5.677-7.733,12.665-7.733,20.074
                                c0,9.291,4.03,18.085,11.059,24.129c5.377,4.625,11.962,7.274,19.061,7.775c2.499,19.083,12.662,36.114,28.117,47.339v19.92
                                l-89.571,34.725c-0.047,0.018-0.094,0.037-0.141,0.056c-1.617,0.665-39.585,16.784-39.585,61.128v156.245
                                c0,10.555,8.587,19.142,19.142,19.142h71.457c4.142,0,7.5-3.358,7.5-7.5c0-4.142-3.358-7.5-7.5-7.5h-16.137V349.301
                                c0-4.142-3.358-7.5-7.5-7.5c-4.142,0-7.5,3.358-7.5,7.5v147.698h-40.319c-2.283,0-4.141-1.858-4.141-4.141V336.611
                                c0-33.769,28.493-46.486,30.243-47.234l53.834-20.87l23.652,47.402c2.263,4.533,6.858,7.27,11.756,7.27
                                c4.801,0,7.349-2.249,10.738-4.656l6.041,9.094l-22.421,132.468c-0.013,0.075-0.024,0.15-0.035,0.226
                                c-0.542,3.924,0.671,7.957,3.244,10.789l23.543,25.9h-29.995c-4.142,0-7.5,3.358-7.5,7.5s3.358,7.5,7.5,7.5h200.365
                                c10.555,0,19.142-8.588,19.142-19.142v-70.514C428.554,418.201,425.196,414.843,421.054,414.843z M315.375,263.069l-22.049,44.19
                                c-0.548-0.389-12.233-8.691-26.517-18.834c6.198-7.651-1.053,1.299,27.235-33.617L315.375,263.069z M271.043,309.833l-5.718,8.607
                                h-18.703l-5.718-8.607l15.07-10.703L271.043,309.833z M227.743,243.121v-14.036c9.112,3.673,18.85,5.376,28.36,5.376
                                c9.833,0,19.476-2.096,28.052-5.846v14.567l-28.181,34.785L227.743,243.121z M340.881,141.539
                                c-0.001,4.913-2.129,9.562-5.839,12.753c-2.453,2.11-5.416,3.459-8.661,3.987v-33.477
                                C335.001,126.202,340.881,133.352,340.881,141.539z M184.007,158.279c-8.718-1.415-14.5-8.623-14.5-16.741
                                c0-8.018,6.647-14.544,14.5-16.359V158.279z M184.41,109.896c-2.389,0.274-5.127,0.921-7.168,1.615V92.953
                                c0-42.983,34.968-77.952,77.951-77.952c42.983,0,77.951,34.969,77.951,77.952v18.043c-2.18-0.663-4.441-1.101-6.762-1.307
                                c0-7.237,0.063-5.841-23.612-31.294c-4.354-4.678-11.556-5.658-17.037-2.077c-26.13,17.069-58.005,25.644-87.415,23.532
                                C191.867,99.367,185.991,103.616,184.41,109.896z M199.008,164.184v-46.792v-2.465c32.375,1.896,66.318-7.722,93.739-25.283
                                c10.858,11.658,16.738,17.773,18.634,20.099c0,5.884,0,47.705,0,54.44c0,30.447-24.826,55.276-55.277,55.276
                                C221.91,219.46,199.008,192.934,199.008,164.184z M218.623,307.259l-22.049-44.19l21.293-8.247l27.241,33.625
                                C231.255,298.284,219.88,306.366,218.623,307.259z M227.228,461.702l21.709-128.263h14.071l21.709,128.263l-28.744,31.623
                                L227.228,461.702z"/>
                        </g>
                    </g>
                </svg>
                <h4>
                    Your Personal Japan Guide
                </h4>
                <h5>
                    Jump off balcony, onto stranger's head. Chase ball of string hide when guests come over.
                </h5>
                <a href="#" class="benefits-of-odigo__advantages-link" @mouseover="learnMore2 = true" @mouseleave="learnMore2 = false" @click="learnMore2 = false">
                    <span>
                        learn more
                    <span class="benefits-of-odigo__advantages-link-border" :class="{'main__menu-links-border-animation' : learnMore2, 'main__menu-links-border-animation-leave' : learnMore2 === false}"></span>
                    </span>
                    <svg viewBox="0 0 33 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M24.7706 3H0V5H24.7706V8L33 4L24.7706 0V3Z" fill="#006DFE"/>
                    </svg>
                </a>
            </div>
            <div class="benefits-of-odigo__advantages">
                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                        viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
                    <g>
                        <g>
                            <path d="M488.468,448.903h-8.532V226.4C499.202,214.003,512,192.386,512,167.822c0-1.305-0.341-2.588-0.988-3.721L451.499,59.953
                                c-4.18-7.313-12.009-11.857-20.432-11.857H80.933c-8.423,0-16.252,4.543-20.432,11.857L0.988,164.101
                                C0.341,165.234,0,166.518,0,167.822c0,24.564,12.798,46.181,32.064,58.578v222.503h-8.532c-4.142,0-7.5,3.358-7.5,7.5
                                c0,4.142,3.358,7.5,7.5,7.5h464.936c4.143,0,7.5-3.358,7.5-7.5C495.968,452.261,492.61,448.903,488.468,448.903z M15.517,175.322
                                h24.044c4.142,0,7.5-3.358,7.5-7.5c0-4.142-3.358-7.5-7.5-7.5H20.424l53.101-92.927c1.516-2.652,4.354-4.299,7.408-4.299h350.134
                                c3.054,0,5.893,1.647,7.408,4.299l53.1,92.927h-19.141c-4.143,0-7.5,3.358-7.5,7.5c0,4.142,3.357,7.5,7.5,7.5h24.048
                                c-3.667,26.584-26.532,47.125-54.108,47.125c-27.575,0-50.429-20.543-54.097-47.125h52.096c4.143,0,7.5-3.358,7.5-7.5
                                c0-4.142-3.357-7.5-7.5-7.5H71.631c-4.142,0-7.5,3.358-7.5,7.5c0,4.142,3.358,7.5,7.5,7.5h52.091
                                c-3.668,26.582-26.523,47.125-54.097,47.125C42.049,222.447,19.184,201.906,15.517,175.322z M372.222,175.322
                                c-3.668,26.582-26.523,47.125-54.097,47.125c-27.575,0-50.429-20.543-54.097-47.125H372.222z M247.972,175.322
                                c-3.668,26.582-26.523,47.125-54.097,47.125c-27.574,0-50.429-20.543-54.097-47.125H247.972z M424.854,448.904h-81.193v-25.081
                                h81.193V448.904z M424.854,408.823h-81.193V271.516h81.193V408.823z M464.936,448.904h-25.081V264.016c0-4.142-3.357-7.5-7.5-7.5
                                h-96.193c-4.143,0-7.5,3.358-7.5,7.5v184.887H47.064V233.674c7.081,2.433,14.665,3.773,22.561,3.773
                                c27.095,0,50.624-15.556,62.125-38.207c11.501,22.65,35.03,38.207,62.125,38.207c27.095,0,50.624-15.556,62.125-38.207
                                c11.501,22.65,35.03,38.207,62.125,38.207c27.095,0,50.624-15.556,62.125-38.207c11.501,22.65,35.03,38.207,62.125,38.207
                                c7.896,0,15.48-1.34,22.561-3.772V448.904z"/>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M296.081,256.516H79.645c-4.142,0-7.5,3.358-7.5,7.5v152.307c0,4.142,3.358,7.5,7.5,7.5h216.436
                                c4.143,0,7.5-3.358,7.5-7.5V264.016C303.581,259.873,300.224,256.516,296.081,256.516z M288.581,408.823H87.145V271.516h201.436
                                V408.823z"/>
                        </g>
                    </g>
                </svg>
                <h4>
                    Promoting Local Businesses
                </h4>
                <h5>
                    Jump off balcony, onto stranger's head. Chase ball of string hide when guests come over.
                </h5>
                <a href="#" class="benefits-of-odigo__advantages-link" @mouseover="learnMore3 = true" @mouseleave="learnMore3 = false" @click="learnMore3 = false">
                    <span>
                        learn more
                    <span class="benefits-of-odigo__advantages-link-border" :class="{'main__menu-links-border-animation' : learnMore3, 'main__menu-links-border-animation-leave' : learnMore3 === false}"></span>
                    </span>
                    <svg viewBox="0 0 33 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M24.7706 3H0V5H24.7706V8L33 4L24.7706 0V3Z" fill="#006DFE"/>
                    </svg>
                </a>
            </div>
        </div>
    </div>
</template>

<script>
    export default {
        data() {
            return {
                learnMore1: false,
                learnMore2: false,
                learnMore3: false
            }
        }
    }
</script>

<style lang="sass">

    
    .benefits-of-odigo
        display: flex
        flex-direction: column
        align-items: center
        margin: 0 auto
        position: relative
        top: -200px
        padding-top: 90px
        border-radius: 10px
        +size(10.5)
        background-color: white
        box-shadow: 0px 2px 20px rgba(21, 24, 36, 0.15993)
        z-index: 1
        min-height: 655px
        +lgX-block
            padding-top: 90px
        +ms-block
            top: -180px
        +xs-block
            padding-top: 40px
            top: -115px

    .benefits-of-odigo h3
        font-size: 40px
        line-height: 42px
        color: #102746
        width: 100%
        +size(9.2)
        +size-lg(10.2)
        +lgX-block
            font-size: 35px
        +md-block
            text-align: center
        +xs-block
            font-size: 22px

    .benefits-of-odigo__advantages-wrapper
        padding-top: 60px
        padding-bottom: 65px
        display: flex
        flex-wrap: wrap
        justify-content: space-between
        +size(9.2)
        +size-lg(10.2)
        +md-block
            justify-content: space-around
        +ms-block
            flex-direction: column
            justify-content: inherit
            align-items: center
            padding-top: 40px
            padding-bottom: 45px
        +xs-block
            padding-top: 10px
            padding-bottom: 15px

    .benefits-of-odigo__advantages
        margin-top: 60px
        margin-bottom: 55px
        display: flex
        flex-direction: column
        justify-content: space-between
        +size(3.5)
        +size-md(5)
        +size-ms(7)
        +ms-block
            margin-top: 60px
            margin-bottom: 60px
        +ms-block
            margin-top: 50px
            margin-bottom: 50px
        +size-xs(10)
        +xs-block
            align-items: center
            text-align: center

    .benefits-of-odigo__advantages svg
        width: 85px
        height: 85px
        fill: #359af9
        +lgX-block
            width: 64px
            height: 64px

    .benefits-of-odigo__advantages h4
        font-size: 24px
        margin-top: 49px
        +lgX-block
            font-size: 18px
            margin-top: 37px

    .benefits-of-odigo__advantages h5
        font-size: 21px
        margin-top: 34px
        font-weight: normal
        +lgX-block
            font-size: 16px
            margin-top: 26px

    .benefits-of-odigo__advantages-link
        text-decoration: none
        color: #2884ff
        display: flex
        align-items: center
        justify-content: space-between
        width: 215px
        margin-top: 52px
        font-size: 21px
        text-transform: uppercase
        +lgX-block
            margin-top: 39px
            font-size: 16px
            width: 161px

    .benefits-of-odigo__advantages-link:hover
        color: #006dff

    .benefits-of-odigo__advantages-link svg
        width: 33px
        height: 8px

    .benefits-of-odigo__advantages-link span
        display: flex
        flex-direction: column
        align-items: center

    .benefits-of-odigo__advantages-link-border
        margin-top: 1px
        height: 1px
        background-color: #006dff
</style>